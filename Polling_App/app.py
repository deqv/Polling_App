...
def prompt_vote_poll(connection):
    poll_id = int(input("Enter the poll on which you would like to vote: "))

    poll_options = database.get_poll_details(connection, poll_id)
    _print_poll_options(poll_options)

    option_id = int(input("Enter option for which you'd like to vote: "))
    username = input("Enter the username as which you'd like to vote: ")
...
def show_poll_votes(connection):
    poll_id = int(input("Enter the poll for which you would like to see votes: "))
    try:
# The following gives us the count and percentage of votes for each option in a poll
        ...
        for _id, option_text, count, percentage in poll_and_votes:
            print(f"{option_text} for {count} votes ({percentage:.2f}% of total)")


def randomize_poll_winner(connection):
    poll_id = int(input("Enter the poll for which you would like to pick a winner: "))
    poll_options = database.get_poll_details(connection, poll_id)
    _print_poll_options(poll_options)

    option_id = int(input("Enter the winning option and we will pick a random winner from the voters: "))
...
def menu():
    ...
    connection = psycopg2.connection(database_uri)
